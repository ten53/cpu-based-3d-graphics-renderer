...
void clip_polygon_against_plane(polygon_t* polygon, int plane) {
    ...
    // static array of inside vertices that will be part of final polygon returned via parameter
    vec3_t inside_vertices[MAX_NUM_POLY_VERTICES];
    tex2_t inside_texcoords[MAX_NUM_POLY_VERTICES];
    int num_inside_vertices = 0;

    // start current vertex with first polygon vertex and texture coordinate
    vec3_t* current_vertex = &polygon->vertices[0];
    tex2_t* current_texcoord = &polygon->texcoords[0];

    // start previous vertex with last polygon vertex and texture coordinate
    ...
    // loop all polygon vertices while current is different than the last one
    while (current_vertex != &polygon->vertices[polygon->num_vertices]) {
        current_dot = vec3_dot(vec3_sub(*current_vertex, plane_point), plane_normal);

        // if changed from inside to outside or from outside to inside
        if (current_dot * previous_dot < 0) {
            ...
            // use lerp formula to get interpolated U and V texture coordinates
    ...
    // copy list of inside vertices into destination polygon (out parameter)
